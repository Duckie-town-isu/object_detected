    ...
    for batch_i, (im, targets, paths, shapes, masks) in enumerate(pbar):
        ...
        for si, (pred, proto) in enumerate(zip(preds, protos)):
            ...
            if single_cls:
                pred[:, 5] = 0
            predn = pred.clone()
            scale_boxes(im[si].shape[1:], predn[:, :4], shape, shapes[si][1])  # native-space predictions
            ...
            # callbacks.run('on_val_image_end', predictions, predn, path, names, im[si])
        ...
        if plots and batch_i < 3:
            ...
            plot_images_and_masks(im, output_to_target(preds, max_det=15), plot_masks, paths,
                                  save_dir / f'val_batch{batch_i}_pred.jpg', names)  # predictions
